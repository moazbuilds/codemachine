export default {
  ...
  steps: [
    ...
    resolveStep('context-manager', { engine: 'cursor', model: 'cheetah' }), // Gather and prepare relevant context from architecture, plan, and codebase for task execution
    resolveStep('code-generation', { engine: 'cursor', model: 'cheetah' }), // Generate code implementation based on task specifications and design artifacts
    resolveStep('cleanup-code-fallback', { engine: 'cursor', model: 'cheetah' }), // Simple cleanup task: delete .codemachine/prompts/code_fallback.md if present
    resolveStep('runtime-prep', { executeOnce: true }), // Generate robust shell scripts for project automation (install, run, lint, test)
    resolveStep('task-sanity-check', { engine: 'cursor', model: 'cheetah' }), // Verify generated code against task requirements and acceptance criteria
    resolveStep('git-commit', { engine: 'cursor', model: 'cheetah' }), // Commit the generated and verified code
